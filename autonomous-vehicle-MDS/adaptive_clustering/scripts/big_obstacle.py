...
def publish_obstacle_msg():
  ...
  bigavoid_pub = rospy.Publisher('/big_obstacle_zone',Int32MultiArray,queue_size = 1)
  ...
  box_bool = False
  is_bool_num = 0
  big_array = Int32MultiArray()
  big_array.data = [0,0,0,0,0]
  ...
  while not rospy.is_shutdown():
    ...
    for box in bboxes:
      if(len(global_path.poses) > 4):
        ...
        for j in range(0 , 6):
            ...
            r_gpath = r_gpath_list[j]
            # if(step_num <= 40 and step_num >= 5):  #170 120
            ...
            if (step_num <= 12 and step_num >= 5):  #170 120
                if(r_gpath.get_contour().contains(point_obj)):
                  big_array.data[0] = 1
                  box_bool = True
            elif (step_num <= 20 and step_num >= 13):  #170 120
                if(r_gpath.get_contour().contains(point_obj)):
                  big_array.data[1] = 1
                  box_bool = True
            elif (step_num <= 28 and step_num >= 21):  #170 120
                if(r_gpath.get_contour().contains(point_obj)):
                  big_array.data[2] = 1
                  box_bool = True
            elif (step_num <= 34 and step_num >= 29):  #170 120
                if(r_gpath.get_contour().contains(point_obj)):
                  big_array.data[3] = 1
                  box_bool = True
            elif (step_num <= 40 and step_num >= 35):  #170 120
                if(r_gpath.get_contour().contains(point_obj)):
                  big_array.data[4] = 1
                  box_bool = True
            else:
    ...
    if(step_num > 40): # 낭떨어지 및 버스 전용 도로 방지 170
      obstacle_msg.obstacles.append(ObstacleMsg())
      obstacle_msg.obstacles[box_num].id = box_num
      v1 = Point32()
    ...
    bigavoid_pub.publish(big_array)
    big_array.data = [0,0,0,0,0]
    # is_bool.data = False

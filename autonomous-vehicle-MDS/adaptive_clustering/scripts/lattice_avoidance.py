...
def publish_obstacle_msg():
    ...
    result = generate_path(states, k0)  # path result by generate_path

    #### length parameter
    obstacle_length = (box.size_x)/2 + v_width
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(len(path_2.poses)-1):
            ...
            height = math.sqrt(dx**2+dy**2)
            #r_gpath = RotatedRect(cx, cy, height, 1.5, theta) 
            r_gpath = RotatedRect(cx, cy, height, obstacle_length, theta) 

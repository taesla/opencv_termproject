...
def publish_obstacle_msg():
    ...
    marker1_pub = rospy.Publisher('visualization_marker_1', MarkerArray, queue_size = 50)
    marker2_pub = rospy.Publisher('visualization_marker_2', MarkerArray, queue_size = 50)
    marker3_pub = rospy.Publisher('visualization_marker_3', MarkerArray, queue_size = 50)
    bool_pub = rospy.Publisher('/adaptive_clustering/is_bool',Bool,queue_size = 1)
    path1_pub = rospy.Publisher('/adaptive_clustering/Path_2',Path,queue_size = 1)
    path2_pub = rospy.Publisher('/adaptive_clustering/Path_3',Path,queue_size = 1)
    path3_pub = rospy.Publisher('/adaptive_clustering/Path_4',Path,queue_size = 1)
    finalpath_pub = rospy.Publisher('/adaptive_clustering/path_final',Path,queue_size = 1)
    #finalpath_pub = rospy.Publisher('/final_path',Path,queue_size = 1)
    ...
    while not rospy.is_shutdown():
        ...
        cx_global = (global_path.poses[1].pose.position.x + global_path.poses[0].pose.position.x)/2
        cy_global = (global_path.poses[1].pose.position.y + global_path.poses[0].pose.position.y)/2
        ...
        for i in range(len(path_1.poses)-1):
            ...
            dx_global = global_path.poses[i+1].pose.position.x - global_path.poses[i].pose.position.x
            dy_global = global_path.poses[i+1].pose.position.y - global_path.poses[i].pose.position.y
            theta_global = math.atan2(dy_global,dx_global)

            cx_global = (global_path.poses[i+1].pose.position.x + global_path.poses[i].pose.position.x)/2
            cy_global = (global_path.poses[i+1].pose.position.y + global_path.poses[i].pose.position.y)/2

            #height = math.sqrt(dx**2+dy**2)
            height = math.sqrt(dx_global**2+dy_global**2)
            #r_gpath = RotatedRect(cx, cy, height, 1.5, theta)
            r_gpath = RotatedRect(cx_global, cy_global, height, 1.5, theta_global)

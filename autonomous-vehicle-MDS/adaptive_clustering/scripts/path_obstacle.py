  ...
  while not rospy.is_shutdown():
    ...
    if(len(global_path.poses) > 4):
      for i in range(0,6):
        ...
        if(step_num < 80): # 동적장애물 step 체크 70
    ...
    for box in bboxes:
      if(len(global_path.poses) > 4):
        ...
        for j in range(0 , 6):
          ...
          r_gpath = r_gpath_list[j]
          # if((step_num <= 140 and step_num >= 130)and(step_num <= 174 and step_num >= 150)):  #170 120
          if(step_num <= 69 and step_num >= 68): #69 65
    ...
    if(step_num > 80): # 낭떨어지 및 버스 전용 도로 방지 170
      obstacle_msg.obstacles.append(ObstacleMsg())
      obstacle_msg.obstacles[box_num].id = box_num
      v1 = Point32()

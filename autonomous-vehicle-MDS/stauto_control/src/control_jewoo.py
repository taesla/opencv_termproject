...
def delivery_finish_callback(data):
    global delivery_finish_sign, delivery_sign_detect
    
    
    delivery_finish_sign=data.data
    print(delivery_finish_sign)
    if(delivery_finish_sign):
        delivery_sign_detect = True
...
if __name__ == '__main__':
    ...
    max_speed=1.8 #3.0#3.5
    min_speed=1.0    #2.5#2.5
    rospy.sleep(1.5)

    parking_finish_time=time.time()
    delivery_finish_time = time.time()
    cruise_over_time = time.time()
    B_zone_time =time.time()
    delivery_sign_detect_time = time.time()
    ...
    while not rospy.is_shutdown():
        ...
        elif((state_machine[5]==1) and (delivery_sign_detect == False)):
            print("delivery_sign_detect : False")
            ackermann.drive.speed = Speed_linear/2
            ackermann.drive.steering_angle = 0.01 #- : left
            ackermann.drive.jerk = 0
            ackermann.drive.acceleration = 0
            if(A_finish==True):
                B_zone_time =time.time()
        elif((state_machine[5]==1) and (delivery_sign_detect == True)):
            print("---------delivery_sign_detect : True-------")
            delivery_sign_detect_time=time.time()
            if (delivery_finish_sign==True) and (delivery_sign_detect_time-cruise_over_time<6):
                print("delivery_finish_sign : ",delivery_finish_sign)
                ackermann.drive.speed = 0
                ackermann.drive.steering_angle = 0.0
                ackermann.drive.jerk = 200
                ackermann.drive.acceleration = 0
                delivery_finish_time=time.time()
            elif (delivery_finish_sign==True) and (delivery_sign_detect_time-B_zone_time<6):
                print("delivery_finish_sign : ",delivery_finish_sign)
                ackermann.drive.speed = 0
                ackermann.drive.steering_angle = 0.0
                ackermann.drive.jerk = 200
                ...
                ackermann.drive.speed = Speed_linear/2
                ackermann.drive.steering_angle = 0.01
                ackermann.drive.jerk = 0
                ackermann.drive.acceleration = 0
                A_finish =True
                print("go")
        ...
        if((prev_state_machine[0]==1 and state_machine[1]==1) or (prev_state_machine[0]==1 and state_machine[3]==1) or (prev_state_machine[0]==1 and state_machine[4]==1) or (prev_state_machine[0]==1 and state_machine[5]==1)):
            ...
            print("stop flag : ",stop_profile_flag)

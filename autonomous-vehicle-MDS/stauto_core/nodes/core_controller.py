...
class CoreController():
    ...
    def cbBackup(self,event_msg):
        ...
        #if (5 <= self.step_num and self.step_num <=50):
        #    self.backup_state = self.Machine_State.avoid_cruise.value
        ...
        #if(26<= self.step_num and self.step_num <=30):    #elif , bring num from git to recovery
        #  self.backup_state = self.Machine_State.safety_zone.value

        #elif(31<=self.step_num and self.step_num<=50): #155
        #    self.backup_state = self.Machine_State.avoid_cruise.value
        #    self.dynamic_obstacle = True                                #dynamic obstacle
        #    if(self.step_num == 25): #155
        #       self.dynamic_obstacle = False
        
        if(2 <= self.step_num and self.step_num <= 25):
            self.backup_state = self.Machine_State.avoid_cruise.value

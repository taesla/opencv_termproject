...
class CoreController():
    ...
    def cbBackup(self,event_msg):
        ...
        if(79 <= self.step_num and self.step_num <= 84): #traffic mission(3)
        ...
        # elif(163 <= self.step_num and self.step_num <= 193): #big_obstacle  mission(5)
        #    self.backup_state = self.Machine_State.avoid_cruise.value

        elif(195 <= self.step_num and self.step_num <= 205): #traffic   mission(6)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 201 and self.step_num == 202):
                self.stop_line_timer = True
                self.stop_line = 1

        # elif (218 <= self.step_num and self.step_num <= 235): #delivery_A   mission(7) (215~235)
        ...
        # elif (397 <= self.step_num and self.step_num <= 420): #delivery_B  mission(9) (395~415)
        #     if(self.delivery_end_B == False):
        #         self.backup_state = self.Machine_State.deliveryB.value
        #     elif(self.delivery_end_B == True):
        #         self.backup_state = self.Machine_State.cruise.value
                
        elif(421 <= self.step_num and self.step_num <= 425): #traffic   mission(10)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 421 and self.step_num == 422):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(486 <= self.step_num and self.step_num <= 492): #traffic   mission(11)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 488 and self.step_num == 489):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(593 <= self.step_num and self.step_num <= 599): #trafficleft   mission(12)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 595 and self.step_num == 596):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(660 <= self.step_num and self.step_num <= 665): #trafficleft   mission(15)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 661 and self.step_num == 662):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(728 <= self.step_num and self.step_num <= 734): #traffic   mission(16)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 730 and self.step_num == 731):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(810 <= self.step_num and self.step_num <= 817): #traffic   mission(17)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 811 and self.step_num == 812):
                self.stop_line_timer = True
                self.stop_line = 1
    ...
    def fnDecideMode(self,mode,sub_event):
        ...
        if self.cur_state == self.Machine_State.traffic.value or sub_event == self.Machine_State.traffic.value:
            ...
            elif (486 <= self.step_num and self.step_num <= 493) and (self.cur_traffic == 1) and (self.stop_line == 1):
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (593 <= self.step_num and self.step_num <= 600) and (self.cur_traffic == 1) and (self.stop_line == 1):
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0
            ...
            if (self.cur_traffic!=0) or (self.traffic_count > 29):
                ...
                if(486 <= self.step_num and self.step_num <= 493):
                ...
                if(593 <= self.step_num and self.step_num <= 600):
                    if(self.cur_traffic ==2 or self.cur_traffic==3):
                        self.stop_flag=False
                        self.cur_state = self.Machine_State.cruise.value
                    elif(self.cur_traffic ==1) and (self.stop_line == 1):

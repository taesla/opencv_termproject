...
class CoreController():
    ...
    def cbDelivery_sign(self,event_msg):
        ...
        if event_msg.data[self.DeliverySign.A1.value] == 1:
            ...
            print('A2',self.DeliverySign.A2.value)
    ...
    def cbBackup(self,event_msg):
        ...
        ###################################### delivery_B ##############################################
        elif (11 <= self.step_num and self.step_num <= 22): 
            print(self.delivery_B_timer)
            print(self.delivery_B_count)

            self.delivery_B_timer = True
            self.delivery_B_count = self.delivery_B_count + 1
            if(self.delivery_B_count > 300):
                    self.delivery_B_timer = False
            if(self.delivery_B_timer == True):
                self.backup_state = self.Machine_State.stop.value
                self.backup_state = self.Machine_State.delivery.value
            elif(self.delivery_B_timer == False):
                self.backup_state = self.Machine_State.cruise.value
        else:
            self.backup_state = self.Machine_State.cruise.value
    ...
    def fnDecideMode(self,mode,sub_event):
        ...
        if self.cur_state == self.Machine_State.delivery.value:                   #////////////////////////////////////////////////////////#
            ...
            if(self.cur_deliverysign == self.DeliverySign.B1.value):

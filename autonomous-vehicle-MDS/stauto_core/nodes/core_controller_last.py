...
class CoreController():
    ...
    def cbBackup(self,event_msg):
        ...
        if (11 <= self.step_num and self.step_num <=31): #parking   mission(1)
            if(self.parking_end == False):
                self.backup_state = self.Machine_State.parking.value
            elif(self.parking_end == True):
                self.backup_state = self.Machine_State.cruise.value

        elif(40 <= self.step_num and self.step_num <= 46): #traffic  mission(3)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 42 and self.step_num == 43):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(59 <= self.step_num and self.step_num <= 65): #traffic  mission(4)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 61 and self.step_num == 62):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(75<= self.step_num and self.step_num <=84):   #safety 75 84
            self.backup_state = self.Machine_State.safety.value

        elif(85<= self.step_num and self.step_num <= 101): #big_obstacle  mission(5)
           self.backup_state = self.Machine_State.avoid_cruise.value
           if(self.step_num == 87):
               self.avoid_timer = True
           elif(self.step_num == 101):
               self.avoid_timer = False

        elif(105 <= self.step_num and self.step_num <= 111): #traffic   mission(6)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 107 and self.step_num == 108):
                self.stop_line_timer = True
                self.stop_line = 1

        elif (114 <= self.step_num and self.step_num <= 132): #delivery_A   mission(7)
            if(self.delivery_end_A == False):
                self.backup_state = self.Machine_State.deliveryA.value
            elif(self.delivery_end_A == True):
                self.backup_state = self.Machine_State.cruise.value

        elif (224 <= self.step_num and self.step_num <= 236): #delivery_B  mission(9)
            if(self.delivery_end_B == False):
                self.backup_state = self.Machine_State.deliveryB.value
            elif(self.delivery_end_B == True):
                self.backup_state = self.Machine_State.cruise.value
                
        elif(237 <= self.step_num and self.step_num <= 243): #traffic   mission(10)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 239 and self.step_num == 240):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(281 <= self.step_num and self.step_num <= 287): #traffic   mission(11)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 283 and self.step_num == 284):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(344 <= self.step_num and self.step_num <= 350): #trafficleft   mission(12)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 346 and self.step_num == 347):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(406 <= self.step_num and self.step_num <= 412): #traffic   mission(15)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 408 and self.step_num == 409):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(450 <= self.step_num and self.step_num <= 456): #traffic   mission(16)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 452 and self.step_num == 453):
                self.stop_line_timer = True
                self.stop_line = 1

        elif(466 <= self.step_num and self.step_num <= 472): #traffic   mission(17)
            self.backup_state = self.Machine_State.traffic.value
            if (self.prev_step == 468 and self.step_num == 469):
                self.stop_line_timer = True
                self.stop_line = 1
    ...
    def fnDecideMode(self,mode,sub_event):
        ...
        if self.cur_state == self.Machine_State.traffic.value or sub_event == self.Machine_State.traffic.value:
            ...
            elif (105 <= self.step_num and self.step_num <= 112) and (self.cur_traffic == 2) and (self.stop_line == 1): #traffic   mission(6)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (237 <= self.step_num and self.step_num <= 244) and (self.cur_traffic == 2) and (self.stop_line == 1): #traffic   mission(10)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (281 <= self.step_num and self.step_num <= 288) and (self.cur_traffic == 2) and (self.stop_line == 1): #traffic   mission(11)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (344 <= self.step_num and self.step_num <= 351) and (self.cur_traffic == 1) and (self.stop_line == 1): #trafficleft   mission(12)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (344 <= self.step_num and self.step_num <= 351) and (self.cur_traffic == 2) and (self.stop_line == 1): #trafficleft   mission(12)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0

            elif (406 <= self.step_num and self.step_num <= 413) and (self.cur_traffic == 2) and (self.stop_line == 1): #trafficleft   mission(15)
                self.stop_flag=True
                self.cur_state = self.Machine_State.stop.value
                self.traffic_timer=True
                self.traffic_count = 0
            ...
            if (self.cur_traffic!=0) or (self.traffic_count > 49):
                print("Traffic Release ", self.traffic_count)

                if(105 <= self.step_num and self.step_num <= 112): #traffic   mission(6)
                ...
                elif(237 <= self.step_num and self.step_num <= 244): #traffic   mission(10)
                    if(self.cur_traffic ==1 or self.cur_traffic==3):
                        self.stop_flag=False
                        self.cur_state = self.Machine_State.cruise.value
                    elif(self.cur_traffic ==2) and (self.stop_line == 1):
                ...
                elif(281 <= self.step_num and self.step_num <= 288): #traffic   mission(11)
                    if(self.cur_traffic ==1 or self.cur_traffic==3):
                        self.stop_flag=False
                        self.cur_state = self.Machine_State.cruise.value
                    elif(self.cur_traffic ==2) and (self.stop_line == 1):
                ...
                elif(344 <= self.step_num and self.step_num <= 351): #traffic   mission(12)
                    if(self.cur_traffic==3):
                        self.stop_flag=False
                        self.cur_state = self.Machine_State.cruise.value
                ...
                elif(406 <= self.step_num and self.step_num <= 413): #traffic   mission(15)
                    if(self.cur_traffic ==1 or self.cur_traffic==3):
                        self.stop_flag=False
                        self.cur_state = self.Machine_State.cruise.value
                    elif(self.cur_traffic ==2) and (self.stop_line == 1):
